...
bool Line::verify() {
    REQUIRE(properlyInitialised(), "The line was not properly initialised.");
    if (trams.empty()) {
        Logger::error("VerificationError: er zijn sporen waarvoor geen tram bestaat");
        verified=false;
    }
    return verified;
}
...
void Line::deployTram(Tram *newTram, const std::string &stationName) {
    REQUIRE(properlyInitialised(), "The line was not properly initialised.");
    std::vector<TramStop*>::iterator it = stations.begin();
    while (it!=stations.end()) {
        TramStop *station = *it;
        if (station->getName()==stationName) {
            if (station->isOccupied()) {
                Logger::error("Deploy error, this station was already occupied.");
                delete newTram;
                verified = false;
                return;
            }
    ...
    Logger::error("StartStation Of Tram Not Found");
    Logger::error("Deploy Error: Niet elke tram heeft een lijn die overeenkomt met een spoor in zijn beginstation");
    verified = false;
    delete newTram;
}
...
void Line::connect(const std::string &start, const std::string &end) {
    REQUIRE(properlyInitialised(), "The line was not properly initialised.");
    TramStop *startStation = getStation(start);
    TramStop *endStation = getStation(end);
    if (startStation==NULL) {
        Logger::error("Failed to connect: StartStation not found in Line");
        Logger::error("Failed to connect: niet elk station is verbonden met een voorgaand en een volgend station voor elk spoor");
        verified = false;
    } else if (endStation==NULL) {
        Logger::error("Failed to connect: EndStation not found in Line");
        Logger::error("Failed to connect: niet elk station is verbonden met een voorgaand en een volgend station voor elk spoor");
        verified = false;
    } else {
